...
class Semeval2017Test(L.LightningDataModule):
    def __init__(
        ...
        self._finbert_dataset: datasets.Dataset = sem_pp_ft.get_dataset(train_dataset=False)
        self._hemlp_dataset: datasets.Dataset = sem_pp_mlp.get_dataset(train_dataset=False)
...
def _collate_fn(raw_samples):
    ...
    tokenizer_output, scores = ft_dm.Semeval2017Test().train_dataloader().collate_fn(raw_samples)
    embeddings, _, new_features = mlp_dm.Semeval2017Test().train_dataloader().collate_fn(raw_samples)
